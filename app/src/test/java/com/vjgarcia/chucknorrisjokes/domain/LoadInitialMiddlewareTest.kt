...
class LoadInitialMiddlewareTest: RxUnitTest() {
    ...
    private val sut = LoadInitialMiddleware(loadJokesMiddleware)
    ...
    fun `doesn't emit jokes if an error happens`() {
        ...
        bindTestObservable
            .assertValues(LoadInitialResult.Loading, LoadInitialResult.Error(anyException))
            .assertComplete()
            .dispose()
    }
    ...
    fun `emits jokes chunks until an error happens`() {
        ...
        bindTestObservable
            .assertValues(
                LoadInitialResult.Loading,
                LoadInitialResult.Next(listOf(anyJoke1)),
                LoadInitialResult.Next(listOf(anyJoke2)),
                LoadInitialResult.Error(anyException)
            )
            .assertComplete()
            .dispose()
    }
    ...
    fun `finishes after emitting all the jokes chunks`() {
        ...
        bindTestObservable
            .assertValues(
                ...
                LoadInitialResult.Next(listOf(anyJoke1)),
                LoadInitialResult.Next(listOf(anyJoke2)),
                LoadInitialResult.Next(listOf(anyJoke3)),
                LoadInitialResult.Complete
            )
            .assertComplete()
            .dispose()
    }
    ...
    @Test
    fun `does nothing if the input actions don't contain LoadInitial`() {
        val anyJoke = anyJoke()
